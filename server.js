...
// Use console logging directly; `logInfo`/`logError` removed.
...
const getApiKey = db.prepare("SELECT * FROM apikeys WHERE key = ?");
const markFlagged = db.prepare("UPDATE flagged SET uid = 0, description = '-' WHERE uid = ?");
// Prepared statement to update an existing flagged entry's description
...
const checkPerms = (requiredPerm) => {
  return (req, res, next) => {
    ...
    const apiKeyRow = getApiKey.get(key);
